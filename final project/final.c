...
void playGame()
{
    ...
    while (gameStatus == 0)///until gameStatus ==0. It means the loop will continue until the game is finished.
    {
        ...
        gotoxy(23,26);
        if(player == 1)
        printf(GREEN"%s turn (Use W/A/S/D or arrow keys to move, and press Enter to select): ", s1);
        else
        printf(GREEN"%s turn (Use W/A/S/D or arrow keys to move, and press Enter to select): ", s2);
        char move = getch();
